...
## Requirements
- [ ] Examine possible dependency loops in implementation files 
...
- [ ] Override Class: Should track who (manager ID), when, what was overridden, and reason. For audit
	- Decide between simple dependency or strategy pattern (consider context, behavior and variety)
- [ ] Controller Interface: Look into APIs/touchscreen for inventory, accounting, payment
	- MockScale fakeScale; InventoryRecord inventoryRecord; Ledger ledgerEntry; CheckoutSystem checkout(fakeScale);
- [ ] Change if check for amount type, doubled
